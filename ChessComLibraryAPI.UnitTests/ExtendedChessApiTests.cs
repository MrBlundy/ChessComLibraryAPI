...
namespace ChessComLibraryAPI.UnitTests
{
    ...
    {
        [TestMethod]
        public void MovesCount_ReturnsInt()
        {
        ...
        [TestMethod]
        public async Task CheckPercentageShortGames_ReturnsDouble()
        {
            var games = await ChessAPI.GetMonthlyArchiveAsync("https://api.chess.com/pub/player/calculatedblunder/games/2019/04");
            var percentage = ExtendedChessApi.CheckPercentageShortGames(games);

            Assert.IsTrue(percentage > 0);
        }

        [TestMethod]
        public async Task CheckPercentageLostGames_ReturnsDouble()
        {
            var games = await ChessAPI.GetMonthlyArchiveAsync("https://api.chess.com/pub/player/calculatedblunder/games/2018/08");
            var percentage = ExtendedChessApi.CheckPercentageLostGames("calculatedblunder", games);
